    ...
    if (page.style.display === "block") {
      ...
      if (page.id === "content-home") {
        history.replaceState(
          {
            div: home.id,
            postViewHeight: screenHeights["content-home"],
            section: "initial",
          },
          ""
        );
  ...
  postVisible.forEach((post) => {
    post.addEventListener("click", (event) => {
      if (event.target.classList.contains("like-btn")) {
        ...
        event.target.classList.contains("follow") ||
        event.target.classList.contains("following")
      ) {
        follow(event.target);
      } else {
        ...
        history.pushState(
          {
            div: post_comment.id,
            postViewHeight: screenHeights["post-comment-view"],
...
function showPosts(allPost) {
  allPost.forEach((element) => {
    postViewAll.innerHTML += `
        ...
        }</span> • <span class="user-name">${showTime(element.created)}</span>
        </p>
        ...
        <button class="follow-btn ${element.id} all ${element.user.userId} ${
      element.followed == "You" ? "following" : element.followed
    }">${element.followed == "You" ? "Edit" : element.followed}</button>
        </section>
        </div>
        ...
        <div>
        <button class="like-btn like-btn-${element.id} ${
      element.liked ? "liked" : "not-liked"
        ...
        <path d="M12 21.35l-.874-.835C5.798 16.182 2 12.015 2 8.5 2 5.42 4.42 3 7.5 3 9.642 3 11.486 4.592 12 6.25 12.514 4.591 14.358 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.515-3.798 7.682-8.126 12.015L12 21.35z" fill="rgb(249, 24, 128)" stroke-width="2"/>
            ...
            : `<svg class="svg-not-liked" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" width="20" height="20">
        <path fill="none" d="M0 0h24v24H0z"/>
        ...
        ${
          element.like_count > 1
            ? element.like_count + " Likes"
            : element.like_count + " Like"
        }</button><br>
...
function postFollowing(post) {
  ...
  post.forEach((element) => {
    postViewFollowing.innerHTML += `
            ...
            <p class="username"> <span>${
              element.user.userName
            }</span> • <span class="user-name">${showTime(
      element.created
    )}</span>
            </p>
            <p class="user-name">${element.user.userEmail}</p>
            ...
            <section class="follow">
            <button class="follow-btn ${element.id} Following ${
      element.user.userId
    } ${element.followed == "You" ? "following" : element.followed}">${
      element.followed == "You" ? "Edit" : element.followed
    }</button>
            </section>
            </div>
      ...
      allLikeButton.forEach((item) => {
        item.innerHTML = `${
          post.liked
            ...
            : `<svg class="svg-not-liked" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20">
                        <path fill="none" d="M0 0h24v24H0z"/>
                        <path d="M12 21.35l-.874-.835C5.798 16.182 2 12.015 2 8.5 2 5.42 4.42 3 7.5 3 9.642 3 11.486 4.592 12 6.25 12.514 4.591 14.358 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.515-3.798 7.682-8.126 12.015L12 21.35z" stroke="rgb(85,85,85)" stroke-width="2"/>
                        </svg>`
        }
...
export { showTime, screenHeights, postInteraction };
